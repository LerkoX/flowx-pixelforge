/**
 * clip-text-encode 画布组件：提示词编辑（textarea）+ 编码状态（cond 对象 ID）。
...
export default function mount(el, props) {
  ...
  const title = h('strong', 'font-size:12px', '📝 CLIP Encode')
  const statusLabel = h('span', 'color:rgba(255,255,255,0.35);margin-left:auto')
  header.append(dot, title, statusLabel)

  // 提示词参数（多行）
  const rawText = (cur.params || {}).text
  let textControl
  if (isWired(rawText)) {
    textControl = wiredControl('text', rawText)
  } else {
    textControl = h('textarea', [
      'width: 100%', 'background: rgba(255,255,255,0.06)',
      'border: 1px solid rgba(255,255,255,0.14)', 'border-radius: 6px',
      'padding: 5px 7px', 'font-size: 11px', 'color: rgba(255,255,255,0.9)',
      'outline: none', 'box-sizing: border-box', 'font-family: inherit',
      'resize: none', 'flex: 1', 'min-height: 0',
    ].join(';'))
    textControl.value = rawText !== undefined ? rawText : ''
    textControl.placeholder = '输入提示词，如 a cat sitting on the moon, masterpiece'
    textControl.addEventListener('change', () => setParam('text', textControl.value))
    refreshers.push(() => {
      textControl.disabled = !editable()
      textControl.style.opacity = editable() ? '1' : '0.55'
      if (document.activeElement === textControl) return
      const nv = (cur.params || {}).text
      textControl.value = nv !== undefined ? nv : ''
    })
  }
  const textField = h('div', 'margin-bottom:6px;flex:1;display:flex;flex-direction:column;min-height:60px')
  const textLabel = h('label', LABEL_CSS, '提示词 text')
  textField.append(textLabel, textControl)
  if (!isWired(rawText)) textControl.style.flex = '1'

  const condLine = h('div', 'color:rgba(255,255,255,0.45);word-break:break-all;flex-shrink:0')

  el.append(header, textField, condLine)
  ...
  function render(p) {
    ...
    const o = p.outputs || {}
    condLine.textContent = o.conditioning ? `cond: ${o.conditioning}` : '等待执行…'
