/**
 * image-upscale 画布组件：放大参数卡 + 输出图在线预览。
...
export default function mount(el, props) {
  ...
  const title = h('strong', 'font-size:12px', '🔎 图像放大')
  const statusLabel = h('span', 'color:rgba(255,255,255,0.35);margin-left:auto')
  header.append(dot, title, statusLabel)

  // 放大参数
  const scaleInp = numberControl('scale', '2.0')
  const whRow = h('div', 'display:flex;gap:6px')
  const wWrap = h('div', 'flex:1')
  const hWrap = h('div', 'flex:1')
  wWrap.append(h('label', LABEL_CSS, '目标宽 width（0=按倍率）'), numberControl('width', '0'))
  hWrap.append(h('label', LABEL_CSS, '目标高 height（0=按倍率）'), numberControl('height', '0'))
  whRow.append(wWrap, hWrap)

  // method 下拉（被 wired 时回退到绑定控件）
  let methodCtl
  {
    const raw = (cur.params || {}).method
    if (isWired(raw)) {
      methodCtl = wiredControl('method', raw)
    } else {
      const sel = h('select', INPUT_CSS)
      METHODS.forEach((m) => {
        const opt = h('option', '', m)
        opt.value = m
        sel.appendChild(opt)
      })
      sel.value = raw || 'lanczos'
      sel.addEventListener('change', () => setParam('method', sel.value))
      refreshers.push(() => {
        sel.disabled = !editable()
        sel.style.opacity = editable() ? '1' : '0.55'
        const nv = (cur.params || {}).method
        if (nv && METHODS.indexOf(nv) >= 0 && document.activeElement !== sel) sel.value = nv
      })
      methodCtl = sel
    }
  }
  ...
  controls.append(
    field('放大倍率 scale', scaleInp),
    whRow,
    h('div', 'height:6px'),
    field('重采样算法 method', methodCtl),
  )
  ...
  const placeholder = h('span', 'color:rgba(255,255,255,0.25)', '等待放大结果…')
  ...
  imgWrap.addEventListener('click', () => {
    ...
    lightbox = buildImageLightbox(img.getAttribute('src'), 'image: ' + (o.image || ''), closeLightbox)
  ...
  img.addEventListener('error', () => {
    ...
    placeholder.textContent = '已放大（无法在线预览，对象 ID 见下方）'
  ...
  function render(p) {
    ...
    if (o.image && base) {
      ...
      img.style.display = 'none'
      placeholder.style.display = ''
      placeholder.textContent = '已放大（service_url 未解析，无法预览）'
      zoomHint.style.display = 'none'
    } else {
      img.style.display = 'none'
      img.removeAttribute('src')
      placeholder.style.display = ''
      placeholder.textContent = p.status === 'running' ? '放大中…' : '等待放大结果…'
      zoomHint.style.display = 'none'
      imgWrap.style.cursor = ''
      imgWrap.title = ''
    }
    imageLine.textContent = o.image ? `image: ${o.image}` : ''
